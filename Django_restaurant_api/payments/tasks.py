...
from django.db.models import OuterRef, Subquery, Sum, Value, F, Q
...
from .services.vpay import _vpay_requery_transaction
from .services.opay import _opay_requery_transaction
from .services.moniepoint import _moniepoint_requery_transaction
from .services.palmpay import _palmpay_requery_transaction
...
def requery_transaction(self):
    try:    
        ...
        for session in pending_sessions:
            handle_retry_query_external_api.delay(session.transaction_reference)
            logger.info(f"Scheduled re-query task for transaction_reference: {session.transaction_reference}")

            

        # Create a group of tasks for all pending sessions
        # tasks_group = group(handle_retry_query_external_api.s(session.transaction_reference) for session in pending_sessions)
...
@shared_task(bind=True, soft_time_limit=200, max_retries=10)
def handle_retry_query_external_api(self, transaction_reference):
    if not transaction_reference:
        return 
    
    session = CheckoutSession.objects.filter(transaction_reference=transaction_reference).only('bank_choice', 'transaction_reference').first()
    missed_webhook=None

    if session.bank_choice == 'vpay':
        logger.info(f"Scheduling re-query for session: {session.bank_choice} with transaction_reference: {transaction_reference}")
        missed_webhook = _vpay_requery_transaction(transaction_reference)
        # this should be carried out for a successfull payment and not for a failed one, we can check the status in the webhook payload and decide to update the session or not,
        #  for now we will update the session with the webhook payload and let the normal flow handle the rest
    

    elif session.bank_choice == 'moniepoint':
        logger.info(f"Scheduling re-query for session: {session.bank_choice} with transaction_reference: {transaction_reference}")
        missed_webhook = _moniepoint_requery_transaction(transaction_reference)
    
    elif session.bank_choice == 'opay':
        logger.info(f"Scheduling re-query for session: {session.bank_choice} with transaction_reference: {transaction_reference}")
        missed_webhook = _opay_requery_transaction(transaction_reference)
    
    elif session.bank_choice == 'palmpay':
        logger.info(f"Scheduling re-query for session: {session.bank_choice} with transaction_reference: {transaction_reference}")
        missed_webhook = _palmpay_requery_transaction(transaction_reference)
    
    if not missed_webhook:
        logger.info("No webhook found for %s", transaction_reference)
        return
    
    try:
        with transaction.atomic():
            session = CheckoutSession.objects.select_for_update().filter(transaction_reference=transaction_reference).first()
            
            if not session:
                return
            
            session.amount_received = None,
            session.is_active = False
            session.bank_fee = None
            session.payment_status = None
            session.webhook_payload = None
            session.paid_at = None
            session.save()

    except SoftTimeLimitExceeded:
        logger.warning(f"Soft time limit exceeded to handle requery for bank: {session.bank_choice} and  transaction_reference: {transaction_reference}, retrying...")
        raise self.retry(exc=SoftTimeLimitExceeded(), countdown=1)

    except Exception as exc:
        logger.error(f"Failed to handle requery for bank: {session.bank_choice} and transaction_reference : {transaction_reference}: {exc}")
        if self.request.retries >= 3:
            logger.error(f"Max retries reached for {transaction_reference}")
            return
