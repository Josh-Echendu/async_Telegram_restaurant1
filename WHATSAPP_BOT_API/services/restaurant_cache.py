        ...
        if cached_time:
            try:
                ...
                if now_day != cached_day:
                    logger.info(f"Day changed for restaurant {phone_id}. Refreshing cache...")
            ...
            for attempt in range(max_retries + 1):  # Retry up to 3 times
                try:
                    ...
                    if res.status_code != 200:
                        logger.info(f"DRF returned {res.status_code} for restaurant {phone_id}")
                        return None
                    ...
                    logger.info(f"Fetched fresh data for restaurant {phone_id}: open_time={data.get('open_time')}, close_time={data.get('close_time')}, is_closed={data.get('is_closed')}")
                    ...
                    try:
                        ...
                        logger.exception(f"❌ User registration error: {error_data}")
                    except:
                    ...
                    logger.exception(f"🌐 Network error on attempt {attempt}: {e}")
