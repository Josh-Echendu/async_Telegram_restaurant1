from config import *
from helper import generate_session_id
from core_telegram_operations import (
    establish_bot_contact, create_telegram_group, add_member_to_group,
    promote_to_admin, send_test_message, leave_group
)


async def setup_restaurant_telegram(restaurant_name: str, bot_username: str, owner_telegram_id: str, owner_name: str = "Restaurant Owner") -> Dict[str, Any]:
    """
    Complete setup for a restaurant's Telegram group
    
    Args:
        restaurant_name: Name of the restaurant
        bot_username: Bot username (with @)
        owner_telegram_id: Owner's Telegram ID or username
    ...
    logger.info(f"🚀 Starting Telegram setup for {restaurant_name} (Session: {session_id})")
    ...
    try:
        ...
        group = await create_telegram_group(group, group_description)
        ...
        owner_permissions = {
            ...
            "can_promote_members": False  # Don't let owners promote others
        }
        ...
        if 'group_id' in locals() and group_id:
            try:
                # Leave the group to avoid orphaned groups
                await leave_group(group_id)
