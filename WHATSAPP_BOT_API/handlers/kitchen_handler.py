...
async def gencode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        try:
            # Extract the first argument and convert to integer
            number = int(context.args[0])
            await update.message.reply_text(f"Generating {number} codes...")
        except ValueError:
            await update.message.reply_text("Please provide a valid number.")
    else:
        await update.message.reply_text("Usage: /gencode <number>")
