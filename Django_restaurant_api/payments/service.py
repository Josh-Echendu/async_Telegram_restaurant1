...
def calculate_payment_amounts(subtotal, service_mode, restaurant):
    ...
    vat_rounded = math.ceil(vat.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
    ...
    card_fee_raw = math.ceil(grand_total * Decimal('0.015'))
...
def pos_payment_amounts(subtotal):
    vat = subtotal * Decimal('0.075')
    vat_rounded = vat.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    pos_total_raw = subtotal + vat_rounded
    pos_total = Decimal(str(math.ceil(float(pos_total_raw))))
